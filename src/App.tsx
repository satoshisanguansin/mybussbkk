...
const App: React.FC = () => {
  ...
  const requestUserLocation = useCallback((onSuccess: (position: GeolocationPosition) => void, onError: () => void) => {
    // Looser options to increase success rate for geolocation.
    ...
    navigator.geolocation.getCurrentPosition(onSuccess,
      (error: GeolocationPositionError) => {
        ...
        alert(`Could not get your location: ${errorMessage}. Please check browser permissions.`);
        onError();
      },
      geoOptions
    );
  ...
  useEffect(() => {
    ...
    requestUserLocation(position => {
       setUserLocation({
        ...
        // Fail silently on initial load, user can click the button if they want it.
    });
